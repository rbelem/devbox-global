...
Transcripts live in the session transcript store (`opencode` sessions; query via `opencode` session commands, not the filesystem). Sessions are keyed by project; never read sessions from unrelated projects.
