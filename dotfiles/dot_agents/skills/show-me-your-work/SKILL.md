...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript from the session transcript store (`opencode` sessions; query via `opencode` session commands, not the filesystem). Don't read sessions from unrelated projects; that reads unrelated private chats. Walk the log against what actually happened:
