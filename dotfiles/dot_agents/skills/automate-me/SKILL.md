---
name: automate-me
description: "Use for \"automate me\", \"create/update/refresh my -mode skill\", \"turn/capture my preferences or working style into a skill\", or wanting agents to follow how the user works. Drafts or revises a personal -mode skill via write-a-skill + unslop, optionally pulling fresh evidence from recent transcripts."
...
Locate the session transcripts before fanning out. Use the session transcript store (`opencode` sessions; query via `opencode` session commands, not the filesystem). Don't read sessions from unrelated projects. That crosses project boundaries and reads private chats from unrelated work.
...
- the `write-a-skill` skill: skill authoring process and writing guidelines.
