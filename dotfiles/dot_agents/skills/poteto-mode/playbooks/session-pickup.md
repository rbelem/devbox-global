...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <transcript path>", "you're taking over", "pick up where X left off", a background-agent URL handoff, or a pushed branch you're meant to continue.
...
1. Locate the prior trail. The session transcript store (`opencode` sessions; query via `opencode` session commands, not the filesystem; do not read sessions from unrelated projects — that crosses project boundaries and reads private chats from unrelated work), a background-agent URL, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
