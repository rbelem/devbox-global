...
- **Worker / verifier.** Prefer a background agent unless the task needs this machine: runtime verification (the `agent-browser` skill for UI surfaces, direct CLI runs for CLIs and TUIs); reading the session transcript store (`opencode` sessions; query via `opencode` session commands, not the filesystem); simulators and local IDE state; auth that exists only here. Background agents cannot see your interactive session, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the background agent's status in the session list. Transcript mtime is not liveness.
