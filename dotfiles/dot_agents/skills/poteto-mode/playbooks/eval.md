...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's transcript from the session transcript store (`opencode` sessions; query via `opencode` session commands, not the filesystem). Do not read sessions from unrelated projects; that crosses project boundaries and reads private chats from unrelated work. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
