...
**Verification driver.** Pick it by surface. Browser, Electron, and web UIs use the `agent-browser` skill. CLIs and TUIs use direct CLI runs. Native mobile uses whatever simulator-driving skill the repo has. A PR that touches two surfaces gets lanes on both. A surface with no way to drive it live is a risk in Appendix C, and its live block still names how each lane drives it.
...
Each live lane runs in its own background agent at the PR head. Drive through the `agent-browser` skill or direct CLI runs.
