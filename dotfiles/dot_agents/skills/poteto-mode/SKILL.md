...
- Any prose surface → the **unslop** skill. Your reply is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the **write-a-skill** skill (for authoring SKILL.md files).
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **technical-writing** skill (`/technical-writing`).
- Before commit → the `stop-slop` skill (`/stop-slop`).
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → the matching verification surface. The `agent-browser` skill drives browser / Electron / web UIs; CLIs and TUIs get direct CLI runs. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
...
- **Session pickup.** Resuming or taking over a prior agent's in-flight work from a transcript, background-agent URL, or pushed branch. `playbooks/session-pickup.md`.
